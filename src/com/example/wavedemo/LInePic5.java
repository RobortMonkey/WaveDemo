...
public class LInePic5 extends View implements Runnable{
	private int vHeight;
	private int vWidth;
	...
	private List<LinePicBean> newData2=new ArrayList<LinePicBean>();//根据originalData构造newData
	private List<Float> XList2=new ArrayList<Float>();
	//---------------------------------------------------------
	private TextPaint verticalTextPaint;
	private Paint horiPaint;//横轴刻度线画笔

	public LInePic5(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public LInePic5(Context context, AttributeSet attrs) {
		this(context,attrs,-1);
	}

	public LInePic5(Context context) {
		this(context,null);
	}
	...
	public void setData2(List<LinePicBean> list)
	{
		...
		regionPath2.moveTo(XList2.get(0), originalData2.get(0).getActualHeight());
		for(int i=1;i<length;i++)
		{
			path2.lineTo(XList2.get(i), originalData2.get(i).getActualHeight());
			regionPath2.lineTo(XList2.get(i), originalData2.get(i).getActualHeight());
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		{
			canvas.drawCircle(XList.get(i), originalData.get(i).getActualHeight() ,radius, circlePoint);
		}
		for(int i=0;i<originalData2.size();i++)//绘制小圆点
		{
			canvas.drawCircle(XList2.get(i), originalData2.get(i).getActualHeight() ,radius, circlePoint2);
		}
	}
    ...
    private void getRegionY(float x)
    {
    	for(int i=0;i<XList.size()-1;i++)
    	{
    		if(x==XList.get(i)||x==XList.get(i+1)||(x>XList.get(i)&&x<XList.get(i+1)))
    		{
    			...
    			Log.e("abc", "-------"+"X1:"+X1+"--------Y1:"+Y1+"--------X2:"+X2+"---------Y2:"+Y2+"-------X:"+x+"------Y:"+Y);
    		}
    	}

...
public class LInePic1 extends View implements Runnable{
	...
	///////////////////////////////////////////
	private Paint pathPaint1;//路径画笔，就是折线
	private Path path1;//line路径
	private Path regionPath1;//闭合区域路径
	private Paint regionPaint1;//区域画笔，画渐变区域
	private Paint verticalLinePaint1;//跟随手指移动的线的画笔
	private Paint circlePaint1;//绘制小圆环画笔
	private Paint circlePoint1;//绘制小圆点画笔
	private List<Float> originalData1;
	private List<Float> newData1=new ArrayList<Float>();//根据originalData构造newData
	private List<Float> XList1=new ArrayList<Float>();
	private float total=800;//代表竖直最大刻度

	public LInePic1(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		this.context=context;
		init();
	}

	public LInePic1(Context context, AttributeSet attrs) {
		this(context,attrs,-1);
	}

	public LInePic1(Context context) {
		this(context,null);
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		vHeight=h;
		vWidth=w;
		LinearGradient gradient = new LinearGradient(0, h, 0, 0, Color.parseColor("#00ffffff"), Color.parseColor("#6629bbea"), TileMode.MIRROR);  
